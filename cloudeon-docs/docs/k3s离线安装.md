...
从节点：node002
## 下载依赖包
链接: [https://pan.baidu.com/s/1CcxlK-oTNzOmSvslNMMGZw](https://pan.baidu.com/s/1CcxlK-oTNzOmSvslNMMGZw) 提取码: 5ats 

本次安装的k3s版本为：v1.23.13
相关包可以在github上release处下载。因为我的服务是x86的，因此下载以下文件即可
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674024568950-9444b6aa-fce6-405d-87b8-861f27798d30.png#averageHue=%23fefefe&clientId=u1bf260ab-9998-4&from=paste&height=897&id=u4f485d3b&name=%E5%9B%BE%E7%89%87.png&originHeight=897&originWidth=1443&originalType=binary&ratio=1&rotation=0&showTitle=false&size=201599&status=done&style=none&taskId=u105d5cce-7a8b-43ac-8437-82db6e4edf7&title=&width=1443)一个是k3s的二进制文件，一个是需要的镜像文件。
...
INSTALL_K3S_SKIP_DOWNLOAD=true INSTALL_K3S_SKIP_START=true ./k3s-install.sh

```
安装成功后，出现下面提示
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674024999108-ae3239bb-03bc-4fe0-858f-73b1b1203b76.png#averageHue=%23103b5a&clientId=uec87f037-a238-4&from=paste&height=338&id=u64d4498c&name=%E5%9B%BE%E7%89%87.png&originHeight=338&originWidth=735&originalType=binary&ratio=1&rotation=0&showTitle=false&size=80692&status=done&style=none&taskId=u0812b830-4575-4c87-81cd-597d8d8c570&title=&width=735)
可以看到，默认会帮我们安装kubectl、contianer
## 主节点启动k3s-server
```shell
...
kubectl get nodes
```
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674025041926-9e62671e-baea-40c2-a346-0b754cb8eb11.png#averageHue=%230b3756&clientId=uec87f037-a238-4&from=paste&height=105&id=u29bcfe16&name=%E5%9B%BE%E7%89%87.png&originHeight=105&originWidth=652&originalType=binary&ratio=1&rotation=0&showTitle=false&size=19319&status=done&style=none&taskId=uc4febca8-cd88-4052-bc06-a6a9c63694a&title=&width=652)



## 从节点安装k3s-agent
从主节点node001将需要的包分发到从节点
```shell
 scp * 172.30.74.20:/opt
```
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674026403148-855f2645-62e7-4954-b2b4-9af66ca7e786.png#averageHue=%23123d5b&clientId=uec87f037-a238-4&from=paste&height=246&id=uda9ce47a&name=%E5%9B%BE%E7%89%87.png&originHeight=246&originWidth=693&originalType=binary&ratio=1&rotation=0&showTitle=false&size=65615&status=done&style=none&taskId=uda1396cd-6c39-439f-a2bf-f64fbeec253&title=&width=693)
登录从节点node002可以看到所需要的文件
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674026443597-1e103467-ecad-4f7b-acec-7a05ab47a76c.png#averageHue=%23053252&clientId=uec87f037-a238-4&from=paste&height=56&id=u118492ca&name=%E5%9B%BE%E7%89%87.png&originHeight=56&originWidth=522&originalType=binary&ratio=1&rotation=0&showTitle=false&size=11246&status=done&style=none&taskId=u1ec5e9c7-69db-420a-b7d1-cfafa06bcf0&title=&width=522)
将文件安装到服务上
```shell
mkdir -p /var/lib/rancher/k3s/agent/images/
sudo cp k3s-airgap-images-amd64.tar /var/lib/rancher/k3s/agent/images/
...
```
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674026558807-f2868bec-775c-46d9-8d78-686cf1232465.png#averageHue=%23103b59&clientId=uec87f037-a238-4&from=paste&height=101&id=u1e37b462&name=%E5%9B%BE%E7%89%87.png&originHeight=101&originWidth=734&originalType=binary&ratio=1&rotation=0&showTitle=false&size=23504&status=done&style=none&taskId=ua0b22153-a70a-452e-9029-256aab85565&title=&width=734)
在从节点node002上执行下面命令，其中172.30.74.19为主节点ip。
```shell
INSTALL_K3S_SKIP_DOWNLOAD=true INSTALL_K3S_SKIP_START=true K3S_URL=https://172.30.74.19:6443 K3S_TOKEN=K1027f5b9bccf92ba6d69f0ff358efd695178e1c0052dd409c4d520198f31526135::server:887b01d35d5c6f9f273db3835dfa6c30 ./k3s-install.sh
```
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674027074030-c0a418cc-42fb-4f89-9b12-f7befb2286a9.png#averageHue=%23113c5a&clientId=uec87f037-a238-4&from=paste&height=414&id=u9e11a610&name=%E5%9B%BE%E7%89%87.png&originHeight=414&originWidth=745&originalType=binary&ratio=1&rotation=0&showTitle=false&size=100222&status=done&style=none&taskId=uf4623a32-56a9-454c-80b7-5fceb3aaffb&title=&width=745)
安装完成后，启动。
```shell
systemctl enable --now k3s-agent
```
成功启动后，可以用kubectl检查。
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674027438245-47f6332e-1b75-4010-bd4e-e46993d23570.png#averageHue=%230e3958&clientId=uec87f037-a238-4&from=paste&height=104&id=u6b11b4e6&name=%E5%9B%BE%E7%89%87.png&originHeight=104&originWidth=611&originalType=binary&ratio=1&rotation=0&showTitle=false&size=21141&status=done&style=none&taskId=ud09e500e-8008-4aaa-bb0e-5668bde7caa&title=&width=611)
## 查看k3s日志
在server节点看日志
```shell
 journalctl -u k3s
...
安装完可以看到如下：
![图片.png](https://cdn.nlark.com/yuque/0/2023/png/637656/1674032965938-f2c42797-e8fd-4279-a8fd-d1fcd32b1d95.png#averageHue=%23fdfefd&clientId=u9d640891-3834-4&from=paste&height=900&id=u24ffeba3&name=%E5%9B%BE%E7%89%87.png&originHeight=900&originWidth=1837&originalType=binary&ratio=1&rotation=0&showTitle=false&size=331774&status=done&style=none&taskId=ud66d6436-dbbe-4ff5-96a3-984c1fabea3&title=&width=1837)
### kuboard卸载
直接执行
```shell
kubectl delete -f https://addons.kuboard.cn/kuboard/kuboard-v3-swr.yaml
```
然后记得删除本地文件/usr/share/kuboard/etcd
