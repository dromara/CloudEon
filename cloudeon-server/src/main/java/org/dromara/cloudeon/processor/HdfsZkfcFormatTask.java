...
public class HdfsZkfcFormatTask extends BaseCloudeonTask {
    ...
    public void internalExecute() {
        ...
        if (runtimeContainer.startsWith("docker")) {
            cmd = String.format("docker run --net=host -v /opt/edp/%s/conf:/opt/edp/%s/conf  %s sh -c \"  yes Y| hdfs --config  /opt/edp/%s/conf zkfc -formatZK \"   ",
                    serviceName, serviceName, stackServiceEntity.getDockerImage(), serviceName);
        } else if (runtimeContainer.startsWith("containerd")) {
            cmd = String.format("ctr run --rm --net-host --mount type=bind,src=/opt/edp/%s/conf,dst=/opt/edp/%s/conf,options=rbind:rw  %s  init  sh -c \"  yes Y| hdfs --config  /opt/edp/%s/conf zkfc -formatZK \"   ",
                    serviceName, serviceName, stackServiceEntity.getDockerImage(), serviceName);
        }
